...
rdkit.initialise().then(() => {
  ...
  app.get("/health", (req, res) => {
    ...
    const id = Math.min(
      responses.length - 1,
      Math.round(Math.random() * responses.length)
    );
  ...
  app.post("/", async (req, res) => {
    ...
    const duplicates = Object.keys(groups).filter(
      (key) => groups[key].length > 1
    );
  ...
  app.post("/validate", async (req, res) => {
    ...
    console.log(`Processing SMILES Validation`);
